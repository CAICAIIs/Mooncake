...
class DynamicReplicationTest : public ::testing::Test {
   protected:
    ...
    // for `proposal_id`. The tenant's lease table is keyed by proposal id, so
    // the caller names the proposal whose lease it wants observed.
    bool HasDynamicState(MasterService& service, const std::string& key,
        ...
        const bool has_lease =
            tenant_handle->FindDynamicReplicationLease(proposal_id).has_value();
        auto has_flags =
            MasterServiceTestPeer(service).WithPublishedObjectForRead(
                TenantId::Default(), key,
                [](const metadata::Tenant&, const std::shared_ptr<ObjectEntry>&,
