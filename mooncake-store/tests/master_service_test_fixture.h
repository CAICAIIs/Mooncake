...
class MasterServiceTest : public ::testing::Test {
   protected:
    ...
    // The bucket count of a tenant's object route. erase() never returns
    // bucket memory, so the post-sweep and post-eviction shrink is only
    ...
    size_t RouteBucketCountForTesting(
        ...
        auto tenant_handle =
            MasterServiceTestPeer::Tenants(service).Lookup(normalized);
        return tenant_handle == nullptr
                   ? 0
                   : tenant_handle->RouteBucketCountForTesting();
    }
    ...
    void SetSoftPinDeadlineForTest(
        ...
        const TenantId normalized_tenant =
            MasterServiceTestPeer(service).ResolveRequestTenantId(
                TenantId(tenant_id));
        auto tenant_handle =
            MasterServiceTestPeer(service).GetOrCreateTenantHandle(
                normalized_tenant);
        auto entry = tenant_handle->Get(key);
    ...
    void ClearGroupStateForTest(MasterService& service) {
        // Drop group membership from each tenant's group index. Membership is
        // keyed by generation, so the generation is read off the entry the
        // callback holds rather than re-read later.
        MasterServiceTestPeer::Tenants(service).Visit(
            [&](const TenantId&,
                ...
                for (const auto& entry : handle->SnapshotObjects()) {
                    ...
                    handle->UnregisterGroupMember(entry, entry->generation());
                }
            });
    }
