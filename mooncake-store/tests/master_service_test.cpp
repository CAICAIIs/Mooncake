...
// The ordering rule for `cleared`: the event means "this tenant is now empty",
// and publishing it after a commit that landed mid-scan would retract an object
// that is live. RemoveAll reads the tenant's epoch before its walk and re-reads
// it when the walk ends, so the commit decides between the two outcomes below.
//
// With no concurrent commit the epoch is unchanged, so the clear must still go
// out. Without this the rule could pass by never publishing.
...
TEST_F(MasterServiceTest, ClearedIsSuppressedWhenACommitRacedTheScan) {
    ...
    const uint64_t baseline =
        MasterServiceTestPeer(service).KvTenantEpochForTesting(tenant);
    ...
    MasterServiceTestPeer(service).PublishKvClearedIfEpochUnchangedForTesting(
        tenant, baseline);
...
// The same rule through the real entry point. RemoveAll reads the epoch
// baseline before the walk, and the entry lock parks the walk inside it, so the
// commit below lands in the window the rule exists for. A run where the sweep
// had not reached the victim yet fails the `racer_key` assertion rather than
// passing vacuously.
TEST_F(MasterServiceTest, RemoveAllWithholdsClearWhenACommitRacesTheScan) {
    ...
    // The sweep takes the victim's entry lock inside its walk, which is exactly
    // the window between the baseline read and the `cleared` decision.
...
TEST_F(MasterServiceTest,
    ...
    // only the published registration is reachable through the registry. A
    // lookup-then-insert pair without one atomic step would let the losers keep
    // writing into a private tenant, silently stranding their objects.
    std::unique_ptr<MasterService> service_(new MasterService());
    [[maybe_unused]] const auto context = PrepareSimpleSegment(*service_);
    const UUID client_id = generate_uuid();
    ...
    // Every object must be reachable through the registry, not stranded in a
    // losing tenant handle.
...
TEST_F(MasterServiceTest, BatchEvictShrinksSparseObjectRoute) {
    ...
    const size_t buckets_before = RouteBucketCountForTesting(*service_);
    ASSERT_GT(buckets_before, kShrinkMinBucketCount);

    MasterServiceTestPeer(*service_).RunBatchEvictForTesting(1.0, 1.0);

    const size_t buckets_after = RouteBucketCountForTesting(*service_);
