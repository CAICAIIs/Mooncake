...
class MasterService {
   ...
   private:
    ...
    // tenant's object route, group table, in-flight replica-action leases and
    // bound quota account.
    ...
    // Candidate state lives in the entry's own state and the tenant keeps only
    ...
    void EraseCandidateLocked(metadata::Tenant& tenant,
    ...
    // The caller passes the envelope and state it holds under the entry's own
    // lock.
    bool HasDynamicReplicationPending(metadata::Tenant& tenant,
                                      const std::shared_ptr<ObjectEntry>& entry,
                                      ObjectEntry::State& state);
