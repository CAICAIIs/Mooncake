...
class MasterService {
   ...
   private:
    ...
    // The entry is handed to `fn` because everything the tenant keys by
    // generation — the group table, the promotion-candidate index, the
    // replica-action leases — is reached through it.
    //
    ...
    [[nodiscard]] auto WithObjectMetadataForWrite(
        ...
        auto outcome = tenant->WithPublishedObject(
            object_id.user_key,
            [&](const std::shared_ptr<ObjectEntry>& entry,
                ObjectMetadata& metadata, ObjectEntry::State& state)
    ...
    [[nodiscard]] auto WithObjectMetadataForRead(
        ...
        return tenant->WithPublishedObjectShared(
