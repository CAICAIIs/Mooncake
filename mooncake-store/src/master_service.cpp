...
MasterService::MasterService(const MasterServiceConfig& config)
    : tenants_([this](const TenantId& tenant_id) {
          ...
          // initialized the same way. The account is resolved here because the
          // quota table keeps one stable account per tenant id and updates it in
          // place, so a tenant's account never changes after it is built.
...
std::vector<std::string> MasterService::PromotionCandidateKeys(
    ...
    keys.reserve(it->second.promotion_candidate_keys.size());
    for (const auto& key : it->second.promotion_candidate_keys) {
        keys.push_back(key);
    }
    return keys;
}
    ...
    const auto outcome = WithObjectMetadataForWrite(
        ...
        [&](metadata::Tenant& tenant, const std::shared_ptr<ObjectEntry>& entry,
            ...
            if (dynamic_copy) {
                ...
                for (const auto& tgt_segment : tgt_segments) {
                    if (!segment_access.ExistsSegmentName(tgt_segment)) {
                        ClearDynamicReplicationStateLocked(tenant_id, entry,
                                                           state);
                        LOG(ERROR)
...
MasterService::SubmitReplicaActionProposalLocked(
    ...
    const auto outcome = WithObjectMetadataForWrite(
        ...
        [&](metadata::Tenant& tenant, const std::shared_ptr<ObjectEntry>& entry,
            ...
            auto existing_lease = FindDynamicReplicationLease(
                object_id.tenant_id, proposal_id);
