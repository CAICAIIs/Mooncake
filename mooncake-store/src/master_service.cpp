...
void MasterService::EraseCandidateLocked(
    ...
    tenant.UnindexPromotionCandidate(entry->key(), entry->generation());
...
void MasterService::RecordOrUpdateCandidateLocked(
    ...
    tenant.IndexPromotionCandidate(key, entry->generation());
    ...
    // much of the cluster one round of retries touches.
    {
        ...
        tenants_.Visit([&](const TenantId& tenant_id,
            ...
            for (const auto& key : tenant->PromotionCandidateKeys()) {
                ...
                (void)tenant->WithPublishedObject(key, [&](const std::
                                                               shared_ptr<
                                                                   ObjectEntry>&
                                                                   entry,
                                                           ObjectMetadata&
...
void MasterService::CleanupExpiredDynamicReplicationState() {
    ...
    tenants_.Visit([&](const TenantId&,
                       const std::shared_ptr<metadata::Tenant>& tenant) {
        // (key, generation at detection): the re-resolution below has to
        // confirm the entry was not torn down and replaced, or the old task's
        // failure would land on the replacement object.
...
MasterService::EvictTenantMemoryForQuota(const TenantId& tenant_id,
    ...
    auto try_evict_group_or_object =
        ...
        std::string group_id;
        const auto outcome = tenant.WithPublishedObject(
            key,
            [&](const std::shared_ptr<ObjectEntry>& entry,
                ObjectMetadata& metadata,
    ...
    std::unordered_set<std::string> blocked_unit_keys;
    {
        ...
        tenants_.Visit([&](const TenantId& tenant_id,
            ...
            for (const auto& snapshot_entry : tenant->SnapshotObjects()) {
                ...
                (void)tenant->WithPublishedObjectShared(
                    snapshot_entry->key(),
                    [&](const std::shared_ptr<ObjectEntry>& entry,
                        const ObjectMetadata& metadata,
